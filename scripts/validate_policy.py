...
RISKS = {"identity", "living_tissue", "child_safety", "exact_text", "logo", "privacy", "external_style"}
...
def main() -> int:
    ...
    require(len(cases) >= 12, "case count")
    ...
    for case in cases:
        ...
        require(expected in {"pass", "reject"}, f"{case_id}: expected")
        require(("stop" in actions) == (expected == "reject") or (expected == "reject" and "request_asset" in actions), f"{case_id}: outcome/action")
        ...
        if "logo" in risks and case.get("permission_status") != "authorized":
    ...
    if errors:
        ...
        return 1
    print("result: PASS")
    print("validated: operational actions, people, text/logo separation, style/reference handling, public fixture hygiene")
    return 0
